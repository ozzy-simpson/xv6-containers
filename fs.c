...
{
	struct inode *ip, *next, *root;

	// Get the root inode, if set
	if (myproc() && myproc()->root != 0)
		root = myproc()->root;
	else
		root = 0;

	// Check if we have a custom root set
	if (*path == '/' && root != 0)
		ip = idup(root);
	else if (*path == '/')
	...
	while ((path = skipelem(path, name)) != 0) {
		...
		// If root set for current process, at root, and trying to .., don't allow
		if (root != 0 && ip == root && namecmp(name, "..") == 0) {
			iunlock(ip);
			continue;
		}
